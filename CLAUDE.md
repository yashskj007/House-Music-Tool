...
| Day 32 | Reliable architecture: replaced 3 Anthropic web-search scout agents (Beatport/Traxsource/RA) + pre-scorer with Last.fm API calls + single final recommendation agent. Discovery 1: server-side fetchLastFmTagTracks() calls tag.gettoptracks for each dominant subgenre in parallel (~1s, no tokens). Discovery 2: client passes _lfmSimilar from analysis phase to /api/recommend; server flattens to unique similar artist list (0 tokens, instant). Discovery 3: CSV niche matcher unchanged. Single Agent Final (2500 tok, no web search, 25s timeout) replaces Agents 12+13. Pipeline: 8 Anthropic calls total (was 13), zero web search calls (was 3), ~30s total (was 60+s). SSE steps updated: prefs → lastfm_tags → lastfm_similar → niche → build. index.html: lfmSimilar now sent in /api/recommend request body. |
| Day 33 | Resilience refinements (10 layers): 1) scoreCandidatesDeterministic() — pure JS scorer, zero deps, always returns 8; 2) LLM as enrichment: deterministic first, 20s LLM enrichment attempt, fallback to deterministic + generateTemplateWhy() on failure; 3) Quorum: MIN_CANDIDATES=15, TARGET_CANDIDATES=50, broaden with 'house' tag if < MIN; 4) Per-step deadlines: STEP_DEADLINES object (prefs 3s, lastfmTags 3s, llmEnrich 20s), skip LLM if pipeline > 40s; 5) cachedFetch() with 1hr TTL Map for Last.fm responses; 6) diversifyCandidates() caps each source at 4 in top-12 pool; 7) logStep() structured JSON logging at every pipeline stage; 8) confidence object {level, reasons} logged for debugging; 9) retryWithJitter() for Last.fm calls (2 attempts, exp backoff + jitter); 10) jobState partial checkpoint object — outer catch returns deterministic results if scorer already ran. Pipeline NEVER fails to produce output. |
| Day 34 | Broader discovery — added MusicBrainz and Discogs as parallel discovery sources alongside Last.fm + CSV. New functions: searchMusicBrainz() (tag-based recording search, 2.5s timeout, 1.1s stagger for rate limit, max 3 subgenres, 1hr cache), searchDiscogs() (style search, 2s timeout, DISCOGS_TOKEN optional env var, 429 rate-limit tracking with 60s block, max 3 subgenres). All 5 sources now run via Promise.allSettled in runDiscovery() helper — parallel execution, graceful per-source failure. Added: dedupeCandidates() (normalize artist+title key, merge also_in[] for cross-source validation), withTimeout() helper, multi-source bonus in scorer (+5 per additional source for cross-validated candidates). diversifyCandidates() updated to cap all 5 sources. Normalized candidate schema across all sources: {artist, title, subgenre, source, tags[], metadata{}}. Source contribution logging added (csv_dependency_pct metric). Timeline steps updated: +musicbrainz, +discogs, +score (scoring) steps. Discovery target: CSV <30% of final 8. Env vars needed: DISCOGS_TOKEN (optional), USER_AGENT_CONTACT (set to yash.skj@gmail.com). |
| Day 35 | Data quality fix — 4-part validation layer: (1) Agent 6 hardened: stricter system prompt with literal JSON example, repairJSON() strips fences/smart-quotes/trailing-commas before re-parse, validateVibeSchema() checks required fields + array sizes, schema-fail triggers one retry at +1500ms with clear error log. (2) Candidate type validation: isValidTrackCandidate() rejects empty titles/placeholder strings, sortCandidates() splits raw pool into trackCandidates + artistHints, enrichArtistHints() resolves up to 12 artist hints into real tracks via Last.fm artist.getTopTracks (4s budget, 1.5s per call timeout) — CSV and lfmSimilar now contribute real track data, not artist-name noise. (3) Final output validator: validateFinalRecommendations() filters recs missing title/artist/URLs/why before browser delivery; degraded mode if <6 remain. (4) callAgent now attempts repairJSON as second-pass parse before throwing. All resilience layers preserved; pipeline still always produces output. |
| Day 31 | SSE keepalive fixes: added 8s keepalive interval (`: keepalive\n\n` comment ping) to both /api/analyse and /api/recommend to prevent Railway 30s idle timeout. Added callAgentWithTimeout() wrapper (25s regular, 55s web-search agents). Graceful scout failure: Agents 8/9/10 each wrapped in try/catch — on timeout/error send `warn` status and empty array, pipeline continues with remaining sources. sendSSE() helper replaces all inline res.write calls. req.on('close') cleans up keepalive interval on client disconnect. readSSE() in index.html gains network/stream retry (up to 2 retries, 2s gap). `warn` status added to timeline CSS (dimmed gold dot). |
